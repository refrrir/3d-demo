...
class PipelineMesh {
    ...
    constructor(pipeline_mesh_input_props: PipelineMeshInputProps[], onClickEvent?: (index:number) => void) {
    ...
    render() {
        ...
        for (let i = 0; i < pipeline_mesh_input_props.length; i++) {
            ...
            pipelines_mesh.setColorAt(i, pipeline_mesh_input_props[i].isPipelineConnected ? COLOR.YELLOW : COLOR.GREY);
